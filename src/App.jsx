...
const Simulador = lazy(() => import('./components/views/Simulador'));
const DreAnaliseCusto = lazy(() => import('./components/views/DreAnaliseCusto'));
const DreCustoLeve = lazy(() => import('./components/views/DreCustoLeve'));
const DreViabilidade = lazy(() => import('./components/views/DreViabilidade'));
const DreGerencial = lazy(() => import('./components/views/DreGerencial'));
const DataImporter = lazy(() => import('./components/views/DataImporter'));
const ConfigFiliais = lazy(() => import('./components/views/ConfigFiliais'));
const ConfigTarifas = lazy(() => import('./components/views/ConfigTarifas'));
const PainelTreinamentos = lazy(() => import('./components/views/PainelTreinamentos'));
const PainelDisponibilidade = lazy(() => import('./components/views/PainelDisponibilidade'));
const GestaoUsuarios = lazy(() => import('./components/views/GestaoUsuarios'));
const Configuracoes = lazy(() => import('./components/views/Configuracoes'));
const GestaoMotoristas = lazy(() => import('./components/views/GestaoMotoristas'));
const DatabaseExplorer = lazy(() => import('./components/views/DatabaseExplorer'));
...
const GestaoFinanceira = lazy(() => import('./components/views/GestaoFinanceira'));
const GestaoOperacional = lazy(() => import('./components/views/GestaoOperacional'));
...
export default function App() {
  ...
  const fetchFromSupabase = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const [
        ...
        extractItems('view_gaps_operacionais')
      ]);
      ...
      setRawOperacionalData(viewGapsOperacionaisRaw || []);
      if (rawOperacionalDataRef) rawOperacionalDataRef.current = viewGapsOperacionaisRaw || [];
      setRawBscData([]);
  ...
  const rawOperacionalDataFiltrado = useMemo(() => {
    return rawOperacionalData.filter(d => 
      (!filtroQuinzenas.length || filtroQuinzenas.includes(d.quinzena)) &&
      (!filtroFiliais.length || filtroFiliais.includes(d.filial)) &&
      (!filtroRegionais.length || filtroRegionais.includes(d.regional)) &&
      (!filtroSupervisores.length || filtroSupervisores.includes(d.supervisor))
    );
  }, [rawOperacionalData, filtroQuinzenas, filtroFiliais, filtroRegionais, filtroSupervisores]);
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8" ref={mainScrollRef}>
          <div className="max-w-7xl mx-auto h-full flex flex-col gap-6">
            ...
            {activeMenu === 'home' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full">
                <HomePage 
                  isOpMode={isOpMode} 
                  currentUser={currentUser} 
                  operacionalSimuladoEvolucao={operacionalSimuladoEvolucao} 
                  dadosFiltradosEvolucao={dadosFiltradosEvolucao} 
                  faturamentoFiltradoEvolucao={faturamentoFiltradoEvolucao} 
                  quinzenasDisponiveis={quinzenasDisponiveis} 
                />
              </div>
            )}
            ...
            {activeMenu === 'gestao_financeira' && (
              <GestaoFinanceira
                resumoMetrics={resumoMetrics}
                pnrTot={pnrTot}
                lostTot={lostTot}
                nvTot={nvTot}
                faturamentoTotalMetrics={faturamentoTotalMetrics}
                margemBrutaMetrics={margemBrutaMetrics}
                setActiveMenu={setActiveMenu}
                prevMargemBrutaMetrics={prevMargemBrutaMetrics}
                prevQuinzenaName={prevQuinzenaName}
                isLoadingDetalhes={isLoadingDetalhes}
                baseRunRateData={baseRunRateData}
                targetQuinzenaRunRate={targetQuinzenaRunRate}
                prevQuinzenaStats={prevQuinzenaStats}
                handleOpenEvolutivo={handleOpenEvolutivo}
                isForecastMode={isForecastMode}
                setIsForecastMode={setIsForecastMode}
                selectedQuinzenaPareto={selectedQuinzenaPareto}
                setSelectedQuinzenaPareto={setSelectedQuinzenaPareto}
                paretoQuinzenaData={paretoQuinzenaData}
                paretoFilialDrilldownData={paretoFilialDrilldownData}
              />
            )}
