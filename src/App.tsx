          ...
          {yAxisSteps.map((step, idx) => {
            ...
            return (
              <div key={`y-axis-${idx}`} className="w-full border-t border-slate-100 flex items-center justify-between" style={{ height: step === 0 ? '0px' : 'auto', marginTop: step === 4 ? '-10px' : '0' }}>
                  <span className="text-[9px] font-bold text-slate-400 bg-transparent pr-2 -translate-y-1/2">{formatAxisVal(valAtStep)}</span>
                  <span className="text-[9px] font-bold text-slate-400 bg-transparent pl-2 -translate-y-1/2">{showFaturamento ? `${((maxRep * (step / 4))).toFixed(1)}%` : ''}</span>
              </div>
            );
          })}
        </div>
              ...
              <div key={`bar-${i}`} className={`flex-1 flex flex-col justify-end h-full relative group max-w-[60px] ${onBarClick ? 'cursor-pointer' : ''}`} onClick={() => onBarClick && onBarClick(d[labelKey])} onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
                ...
                {showFaturamento && (
                    <div className="absolute w-2 h-2 sm:w-2.5 sm:h-2.5 bg-slate-900 rounded-full border-2 border-violet-500 shadow-sm left-1/2 -translate-x-1/2 z-30 transition-all group-hover:scale-150 flex justify-center" style={{ bottom: `calc(${repPct}% - 4px)` }}>
                      <span className="absolute bottom-full mb-1 text-[9px] font-bold text-violet-300 bg-slate-800 px-1 py-0.5 rounded shadow-sm whitespace-nowrap pointer-events-none">{d[labelKey]}</span>
                    </div>
                )}
                <div className="absolute top-full mt-3 w-full text-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold truncate">{d[labelKey]}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="flex-1 flex relative mt-2">
        <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
          {yAxisSteps.map((step, idx) => {
            ...
            return (
              <div key={`y-axis-${idx}`} className="w-full border-t border-slate-100 flex items-center justify-between" style={{ height: step === 0 ? '0px' : 'auto', marginTop: step === 4 ? '-10px' : '0' }}>
                  <span className="text-[9px] font-bold text-slate-400 bg-transparent pr-2 -translate-y-1/2">{formatAxisVal(valAtStep)}</span>
                  <span className="text-[9px] font-bold text-slate-400 bg-transparent pl-2 -translate-y-1/2">{`${dsSteps[idx]}%`}</span>
              </div>
            );
          })}
        </div>
        <div className="z-10 flex w-full h-full items-end justify-around gap-1 sm:gap-2 mx-10 sm:mx-12 border-b border-slate-300 relative">
          <div className="absolute w-full border-t-[3px] border-dashed border-slate-600 z-10 pointer-events-none opacity-60 flex items-center" style={{ bottom: `${((98.5 - 80) / 20) * 100}%` }}>
             <span className="absolute left-0 -ml-10 text-[9px] font-black text-white bg-slate-700 px-1.5 py-0.5 rounded shadow-sm -translate-y-1/2">META</span>
          </div>
          <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible z-20" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            return (
              <div key={`bar-ds-${i}`} className={`flex-1 flex flex-col justify-end h-full relative group max-w-[60px] ${onBarClick ? 'cursor-pointer' : ''}`} onClick={() => onBarClick && onBarClick(d[labelKey])} onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
                ...
                <div className={`absolute w-3 h-3 sm:w-3.5 sm:h-3.5 bg-slate-900 rounded-full border-[3px] shadow-md left-1/2 -translate-x-1/2 z-30 transition-all group-hover:scale-150 flex justify-center ${dotColor}`} style={{ bottom: `calc(${dsDisplayPct}% - 6px)` }}>
                  <span className="absolute bottom-full mb-1 text-[9px] font-bold text-slate-200 bg-slate-800 px-1 py-0.5 rounded shadow-sm border border-slate-700 pointer-events-none">{d[labelKey]}</span>
                </div>
                <div className="absolute top-full mt-3 w-full text-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold truncate">{d[labelKey]}</p>
                </div>
    ...
    <div className="w-full h-[320px] flex flex-col pt-6 pb-12 relative">
      <div className="flex-1 flex relative mt-2">
        <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
          {yAxisSteps.map((step, idx) => (
              <div key={`y-axis-${idx}`} className="w-full border-t border-slate-100 flex items-center justify-between" style={{ height: step === 0 ? '0px' : 'auto', marginTop: step === 4 ? '-10px' : '0' }}>
                  <span className="text-[9px] font-bold text-slate-400 bg-transparent pr-2 -translate-y-1/2">{formatAxisVal(maxVol * (step / 4))}</span>
                  <span className="text-[9px] font-bold text-slate-400 bg-transparent pl-2 -translate-y-1/2">{`${dsSteps[idx]}%`}</span>
              </div>
          ))}
        </div>
...
// ============================================================================
// COMPONENTE: GESTÃO DE PENALIDADES (Visão Sem Faturamento)
// ============================================================================
const RunRatePenalidadesSection = ({ baseData, targetQuinzena, prevStats }) => {
  ...
  if (prevStats) {
      ...
      const todasFiliaisComparadas = [...projFilialData].map(filialAtual => {
          ...
          return {
              ...filialAtual,
              varPen: penAtual - penAnterior, // Diferença em R$
...
export default function App() {
  ...
  const insucessosDisponiveis = useMemo(() => {
    const keys = new Set();
    if (activeMenu === 'gestao_bsc' || activeMenu === 'comparativo_bsc' || activeMenu === 'gaps_operacionais' || activeMenu === 'gestao_penalidades') {
  ...
  const targetQuinzenaRunRate = useMemo(() => {
    ...
    else if (activeMenu === 'gestao_penalidades') relevantData = dadosFiltrados;
    else if (['gestao_bsc', 'comparativo_bsc', 'gaps_operacionais'].includes(activeMenu)) relevantData = bscFiltrado;
  ...
  }, [filtroQuinzenas, activeMenu, faturamentoFiltrado, bscFiltrado, operacionalFiltrado, dadosFiltrados, quinzenasDisponiveis]);
        ...
        <nav className="flex-1 py-6 flex flex-col gap-6 px-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <button onClick={() => handleMenuChange('gaps_operacionais')} className={`w-full flex items-center justify-start text-left gap-3 px-3 py-2.5 rounded-lg font-medium transition-colors ${activeMenu === 'gaps_operacionais' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
              ...
              <span className="truncate">Insucessos</span>
            </button>
          </div>
        </nav>
      </aside>
