...
if (idx5 !== -1) {
  // We need to find the specific grid that has Penalidades vs Faturamento.
  // Wait, let's just find the exact text: "Penalidades vs Faturamento"
  ...
  if (found) {
    // Find where this grid ends
    // It's 2 divs deep
    let endGrid = gridStart + 1;
    let divCount = 1; // We matched the first <div
    while (divCount > 0 && endGrid < lines.length) {
      if (lines[endGrid].includes('<div')) divCount += (lines[endGrid].match(/<div/g) || []).length;
      if (lines[endGrid].includes('</div')) divCount -= (lines[endGrid].match(/<\/div/g) || []).length;
      endGrid++;
    }
...
console.log("ALL RESTORATIONS DONE.");
